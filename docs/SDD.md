...
An IT support chat agent: an employee asks a question or requests an action in a Streamlit chat UI, a FastAPI backend runs the request through a LangGraph-orchestrated pipeline (redaction → injection guard → session + long-term memory → intent routing → RAG / tool / remember-fact / direct / escalate → response), and the result renders back as either a grounded text answer or a colored tool-result card. See `docs/architecture.md` for the rendered diagrams (`docs/architecture.png`, `docs/langgraph_state.png`).
...
| Long-Term Memory | `src/agent/long_term_memory.py` | Per-user ChromaDB collection, persisted on disk, explicit "remember that ..." trigger, survives a backend restart |
...
- **`MemoryRecord`** — `memory_id`, `tenant_id`, `ticket_id`, `memory_scope` (`MemoryScope`: ephemeral/session/ticket), `key`, `value`, `expires_at`, `created_at`. Written for the original spec's memory design; the memory actually shipped (both session and long-term) uses its own simpler, purpose-built storage instead (`src/agent/memory.py`, `src/agent/long_term_memory.py`) rather than this Pydantic model — see §8.
...
- **`AgentState`** (internal, `TypedDict`, not a wire schema) — `user_id`, `session_id`, `sanitized_message`, `intent`, `retrieved_context`, `tool_result`, `response`, `session_history`, `user_facts`.
...
{
  "response": "string",
  "intent": "policy_question | action_request | escalation | blocked | remember_fact | direct_response",
...
3. `load_session_memory` — loads this session's last 6 turns from `src/agent/memory.py`, and separately recalls any relevant long-term facts for this `user_id` from `src/agent/long_term_memory.py` (only reached on the safe path). Either recall failing (e.g. no ChromaDB index yet) degrades to an empty list rather than breaking the turn.
...
5. Branch by intent:
   ...
   - `escalation` → `escalate`.
   - `remember_fact` (triggered by "remember that ..." / "remember my ..." / "please remember ...") → `remember_fact`, which persists the message to long-term memory.
...
`generate_grounded_answer` is the only node that calls an LLM (NVIDIA NIM/Nemotron via `src/llm/client.py`), and only after RAG has already retrieved and relevance-filtered the policy context — the model's job is strictly "phrase this retrieved text," never "answer from general knowledge." Session turns and recalled long-term facts are both injected into the prompt as clearly-labeled "context only, never a source of policy" blocks, so personalization can shape phrasing without ever overriding grounding. On any API failure or a degenerate/garbled response (`_looks_like_valid_answer()` check in `graph.py`), it silently falls back to the deterministic template built from the same retrieved text, so a bad model sample can never surface as a worse answer than the non-LLM baseline.
...
- **RAG grounding cutoff**: `MAX_RELEVANT_DISTANCE = 1.2` in `src/rag/retrieve.py` — chunks past this ChromaDB distance are dropped, so an off-topic question (no relevant policy in the corpus) escalates instead of confidently answering from the nearest-but-wrong document. `src/agent/long_term_memory.py` applies the same idea with its own calibrated threshold (`1.6`, higher because short single-sentence facts score higher distances than multi-sentence policy chunks even when genuinely relevant).
- **Long-term memory is opt-in and per-user**: only stored on an explicit "remember that ..." request (never inferred), and isolated by a per-user ChromaDB collection (`user_memory_{user_id}`) — verified live that a different user asking an identical question gets zero personalization.

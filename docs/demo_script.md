...
> 12. **A PII-redacted VPN question wrongly escalated instead of answering** (found during a live dry run 2026-09-04): the `[EMAIL_REDACTED]`/`[PHONE_REDACTED]` placeholder tokens got embedded as part of the RAG search query, diluting similarity enough to push an otherwise-clear VPN question (0.88 distance alone) just over the 1.2 relevance cutoff (1.22). Fixed by stripping redaction placeholder tokens before building the RAG/long-term-memory search query only — the real tokens still appear in the LLM prompt, the API response, and memory, since they matter there as a visible record that redaction happened.
> 13. **Also added while dry-running Scene 4**: the sanitized (PII-redacted) message wasn't visible anywhere in the chat UI, only via a separate terminal command. `frontend/app.py` now shows a small "🔒 PII redacted before processing: ..." caption under your own message whenever redaction actually changed something, so this is visible directly in the demo without switching to a terminal.
...
| Backend seems to be running old behavior after a restart | On Windows, `--reload` spawns a second worker process. Always stop it with **Ctrl+C in that exact terminal window**, not by closing the window or killing a different process — closing it the wrong way can leave the worker running in the background on the same port, silently serving stale code. If in doubt, close that terminal entirely and open a fresh one. |
